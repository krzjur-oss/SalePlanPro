import{r as k,j as e}from"./vendor-react-NQNxyWao.js";import{f as rt,a as Ke,c as Ue}from"./index-CHt0Pm7t.js";import{P as E,Z as je,d as nt,X as lt}from"./vendor-lucide-BhYI05HQ.js";import"./vendor-motion-FDUFV107.js";const L=["Poniedziałek","Wtorek","Środa","Czwartek","Piątek"];function Ye(p,M){const w=[];let A=null;return p.forEach((z,q)=>{const S=z.floor.buildingIdx,T=M[S],P=(T==null?void 0:T.name)||"",G=z.floor.id,C=z.floor.name,K=`${S}|${G}`;!A||A.key!==K?(A={startIdx:q,span:1,key:K,name:C,buildingName:P},w.push(A)):A.span++}),w}function He(p){const M=[];let w=null;return p.forEach((A,z)=>{var C,K;const q=A.floor.buildingIdx,S=A.floor.id,T=((C=A.seg)==null?void 0:C.id)||"default_seg",P=((K=A.seg)==null?void 0:K.name)||"Główny",G=`${q}|${S}|${T}`;!w||w.key!==G?(w={startIdx:z,span:1,key:G,name:P},M.push(w)):w.span++}),M}function mt({appState:p,schedData:M}){var Le;const[w,A]=k.useState("classes"),[z,q]=k.useState("etap1"),[S,T]=k.useState("all"),[P,G]=k.useState("all"),[C,K]=k.useState("all"),[Be,H]=k.useState(!1),[Fe,we]=k.useState(!1),[xe,Ne]=k.useState(!1),[Q,ke]=k.useState("landscape"),[Ze,ae]=k.useState(!1),[me,Ve]=k.useState(1),[be,_e]=k.useState("all");k.useEffect(()=>{try{we(window.self!==window.top)}catch{we(!0)}},[]),k.useEffect(()=>{if(xe){let t=document.querySelector('meta[name="viewport"]');const d=t?t.getAttribute("content"):"";t||(t=document.createElement("meta"),t.setAttribute("name","viewport"),document.head.appendChild(t));const s=Q==="landscape"?"1024":"768";t.setAttribute("content",`width=${s}, initial-scale=0.8, shrink-to-fit=no`);const c=document.createElement("style");return c.id="print-mobile-viewport-adjustments",c.innerHTML=`
        @media print {
          html, body {
      ...
      `,document.head.appendChild(c),()=>{t&&(d?t.setAttribute("content",d):t.removeAttribute("content"));const l=document.getElementById("print-mobile-viewport-adjustments");l&&l.remove()}}},[xe,Q]);const m=p.planLekcji,X=k.useMemo(()=>{const t=p.yearLabel||"",d=t.match(/(\d{4})/);let s=new Date().getFullYear(),c=s+1;if(d){s=parseInt(d[1],10);const l=t.match(/\d{4}.*?(\d{4})/);l?c=parseInt(l[1],10):c=s+1}return{start:`${s}-09-01`,end:`${c}-06-25`}},[p.yearLabel]),[ue,ve]=k.useState(X.start),[he,ze]=k.useState(X.end),[fe,Je]=k.useState("[Przedmiot] - [Klasa] [Sala]");k.useEffect(()=>{ve(X.start),ze(X.end)},[X]);const $e=(t,d)=>{const s=new Date(t),c=d+1,l=s.getDay();let n=c-l;n<0&&(n+=7);const r=new Date(s);return r.setDate(s.getDate()+n),r},B=t=>{const d=t.getFullYear(),s=String(t.getMonth()+1).padStart(2,"0"),c=String(t.getDate()).padStart(2,"0"),l=String(t.getHours()).padStart(2,"0"),n=String(t.getMinutes()).padStart(2,"0"),r=String(t.getSeconds()).padStart(2,"0");return`${d}${s}${c}T${l}${n}${r}`},Ae=t=>{const d=t.getFullYear(),s=String(t.getMonth()+1).padStart(2,"0"),c=String(t.getDate()).padStart(2,"0");return`${d}${s}${c}T235959Z`},F=t=>t?t.replace(/\\/g,"\\\\").replace(/,/g,"\\,").replace(/;/g,"\\;").replace(/\n/g,"\\n").trim():"",Ie=["MO","TU","WE","TH","FR"],Pe=t=>{const d=[];for(let s=0;s<5;s++)W.forEach((c,l)=>{const n=String(c.num);let r=[];z==="etap1"?Object.entries(m.lessons).forEach(([a,o])=>{var u,g;const b=a.split("|"),i=b[0],x=parseInt(b[1],10),f=parseInt(b[2],10);if(x===s&&f===l){const h=m.assignments.find(N=>N.id===o.assignmentId);if(h&&h.teacherId===t.id){const N=((u=U.get(h.subjectId))==null?void 0:u.name)||"Inny",$=((g=ee.get(i))==null?void 0:g.name)||"Inna",v=h.roomId?te.get(h.roomId):null;r.push({subject:N,className:$,roomName:v==null?void 0:v.name})}}}):(((Z.teachers[t.id]||{})[s]||{})[n]||[]).forEach(i=>{var x;r.push({subject:i.subject,className:i.className||((x=i.classes)==null?void 0:x.join("+"))||"Klasa",roomName:i.note})}),r.forEach(a=>{d.push({dayIdx:s,hourNum:c.num,start:c.start,end:c.end,subject:a.subject,className:a.className,roomName:a.roomName})})});return d},Ce=t=>{const d=Pe(t);if(d.length===0){alert(`Nauczyciel ${t.last} ${t.first} nie ma przypisanych żadnych lekcji w wybranym planie.`);return}let s=["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//SalePlan Pro//NONSGML v1.0//PL","CALSCALE:GREGORIAN","METHOD:PUBLISH"];d.forEach(a=>{const o=$e(ue,a.dayIdx),[b,i]=a.start.split(":").map(Number),[x,f]=a.end.split(":").map(Number),u=new Date(o);u.setHours(b,i,0,0);const g=new Date(o);g.setHours(x,f,0,0);const h=fe.replace("[Przedmiot]",a.subject).replace("[Klasa]",a.className).replace("[Sala]",a.roomName?`s. ${a.roomName}`:"").replace(/\s+/g," ").trim(),N=`Lekcja: ${a.hourNum} (${a.start}-${a.end})\\nNauczyciel: ${t.last} ${t.first} (${t.abbr})\\nKlasa: ${a.className}\\n`+(a.roomName?`Sala: ${a.roomName}\\n`:"")+"Wygenerowano automatycznie z SalePlan Pro",$=a.roomName?`Sala ${a.roomName}`:"",v=`asg-${t.id}-${a.dayIdx}-${a.hourNum}-${a.className.replace(/[^a-zA-Z0-9]/g,"")}-${Date.now()}@saleplan.pro`,I=new Date(he);s.push("BEGIN:VEVENT"),s.push(`UID:${v}`),s.push(`DTSTAMP:${B(new Date)}Z`),s.push(`DTSTART:${B(u)}`),s.push(`DTEND:${B(g)}`),s.push(`RRULE:FREQ=WEEKLY;UNTIL=${Ae(I)};BYDAY=${Ie[a.dayIdx]}`),s.push(`SUMMARY:${F(h)}`),s.push(`LOCATION:${F($)}`),s.push(`DESCRIPTION:${F(N)}`),s.push("END:VEVENT")}),s.push("END:VCALENDAR");const c=s.join(`\r
`),l=new Blob([c],{type:"text/calendar;charset=utf-8"}),n=URL.createObjectURL(l),r=document.createElement("a");r.href=n,r.download=`plan_${t.last}_${t.first}.ics`,document.body.appendChild(r),r.click(),document.body.removeChild(r),URL.revokeObjectURL(n)},qe=()=>{let t=["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//SalePlan Pro//NONSGML v1.0//PL","CALSCALE:GREGORIAN","METHOD:PUBLISH"],d=0;if(m.teachers.forEach(r=>{Pe(r).forEach(o=>{d++;const b=$e(ue,o.dayIdx),[i,x]=o.start.split(":").map(Number),[f,u]=o.end.split(":").map(Number),g=new Date(b);g.setHours(i,x,0,0);const h=new Date(b);h.setHours(f,u,0,0);const N=`[${r.abbr}] `+fe.replace("[Przedmiot]",o.subject).replace("[Klasa]",o.className).replace("[Sala]",o.roomName?`s. ${o.roomName}`:"").replace(/\s+/g," ").trim(),$=`Nauczyciel: ${r.last} ${r.first} (${r.abbr})\\nLekcja: ${o.hourNum} (${o.start}-${o.end})\\nKlasa: ${o.className}\\n`+(o.roomName?`Sala: ${o.roomName}\\n`:"")+"Wygenerowano automatycznie z SalePlan Pro",v=o.roomName?`Sala ${o.roomName}`:"",I=`asg-all-${r.id}-${o.dayIdx}-${o.hourNum}-${o.className.replace(/[^a-zA-Z0-9]/g,"")}-${Date.now()}-${d}@saleplan.pro`,j=new Date(he);t.push("BEGIN:VEVENT"),t.push(`UID:${I}`),t.push(`DTSTAMP:${B(new Date)}Z`),t.push(`DTSTART:${B(g)}`),t.push(`DTEND:${B(h)}`),t.push(`RRULE:FREQ=WEEKLY;UNTIL=${Ae(j)};BYDAY=${Ie[o.dayIdx]}`),t.push(`SUMMARY:${F(N)}`),t.push(`LOCATION:${F(v)}`),t.push(`DESCRIPTION:${F($)}`),t.push("END:VEVENT")})}),d===0){alert("Brak przypisanych lekcji w całym planie lekcji.");return}t.push("END:VCALENDAR");const s=t.join(`\r
`),c=new Blob([s],{type:"text/calendar;charset=utf-8"}),l=URL.createObjectURL(c),n=document.createElement("a");n.href=l,n.download="plan_wszyscy_nauczyciele.ics",document.body.appendChild(n),n.click(),document.body.removeChild(n),URL.revokeObjectURL(l)},ee=k.useMemo(()=>new Map(m.classes.map(t=>[t.id,t])),[m.classes]),oe=k.useMemo(()=>new Map(m.teachers.map(t=>[t.id,t])),[m.teachers]),U=k.useMemo(()=>new Map(m.subjects.map(t=>[t.id,t])),[m.subjects]),te=k.useMemo(()=>new Map(m.rooms.map(t=>[t.id,t])),[m.rooms]),W=k.useMemo(()=>m.hours&&m.hours.length>0?m.hours:[{num:1,start:"08:00",end:"08:45"},{num:2,start:"08:55",end:"09:40"},{num:3,start:"09:50",end:"10:35"},{num:4,start:"10:55",end:"11:40"},{num:5,start:"11:50",end:"12:35"}],[m.hours]),Z=k.useMemo(()=>{const t=p.yearKey,d=M[t]||{},s={},c={},l={};return Object.entries(d).forEach(([n,r])=>{const a=parseInt(n,10);Object.entries(r).forEach(([o,b])=>{Object.entries(b).forEach(([i,x])=>{(Array.isArray(x)?x:[x]).forEach(u=>{var $,v,I;if(!u)return;const g=i.split("_"),h=g[g.length-1]||"";if(u.classes&&u.classes.length>0)u.classes.forEach(j=>{var D;const y=((D=m.classes.find(se=>se.name===j))==null?void 0:D.id)||j;s[y]||(s[y]={}),s[y][a]||(s[y][a]={}),s[y][a][o]||(s[y][a][o]=[]),s[y][a][o].push({...u,note:h})});else if(u.className){const j=(($=m.classes.find(y=>y.name===u.className))==null?void 0:$.id)||u.className;s[j]||(s[j]={}),s[j][a]||(s[j][a]={}),s[j][a][o]||(s[j][a][o]=[]),s[j][a][o].push({...u,note:h})}const N=u.teacherAbbr;if(N){const j=((v=m.teachers.find(y=>y.abbr===N))==null?void 0:v.id)||N;c[j]||(c[j]={}),c[j][a]||(c[j][a]={}),c[j][a][o]||(c[j][a][o]=[]),c[j][a][o].push({...u,note:h})}if(h){const j=((I=m.rooms.find(y=>y.name===h))==null?void 0:I.id)||h;l[j]||(l[j]={}),l[j][a]||(l[j][a]={}),l[j][a][o]||(l[j][a][o]=[]),l[j][a][o].push({...u,note:h})}})})})}),{classes:s,teachers:c,rooms:l}},[M,p.yearKey,m.classes,m.teachers,m.rooms]),Qe=()=>{window.print()},Xe=()=>{const t=Ee,d=Math.max(...t.map(l=>l.cols.length),1),s=Math.min(1,Math.max(.45,12/d));let c="";return[0,1,2,3,4].forEach(l=>{let n="";t.forEach(r=>{const a=r.cols.length,o=Ye(r.cols,p.buildings),b=He(r.cols);let i="110px",x="8px",f="8px",u="10px",g="9px",h="8.5px",N="12px",$="8.5px",v=!0,I="130px";a>24?(i="45px",x="3px 1px",f="3px 1px",u="7.5px",g="7px",h="6.5px",N="8px",v=!1,I="50px"):a>16?(i="65px",x="4px 2px",f="4px 2px",u="8.5px",g="8px",h="7.5px",N="9.5px",v=!1,I="75px"):a>10?(i="85px",x="6px 3px",f="6px 3px",u="9px",g="8.5px",h="8px",N="11px",$="7.5px",I="100px"):a>6&&(i="100px",x="8px 4px",f="8px 4px",u="9.5px",g="9px",h="8px",N="12px",$="8px",I="115px");let j="";W.forEach(y=>{const D=W.findIndex(R=>R.num===y.num);let se="";r.cols.forEach(R=>{var Me,Re,Oe;let V=[];if(z==="etap1")Object.entries(m.lessons).forEach(([O,_])=>{var Te,We,Ge;const de=O.split("|"),Y=de[0],ce=parseInt(de[1],10),st=parseInt(de[2],10);if(ce===l&&st===D){const J=m.assignments.find(pe=>pe.id===_.assignmentId);if(J){const pe=m.rooms.find(ye=>ye.id===J.roomId);if(J.roomId===R.room.id||pe&&pe.name.toLowerCase().trim()===R.room.num.toLowerCase().trim()){const ye=((Te=U.get(J.subjectId))==null?void 0:Te.name)||"Przedmiot",at=((We=ee.get(Y))==null?void 0:We.name)||"Klasa",ot=J.teacherId?(Ge=oe.get(J.teacherId))==null?void 0:Ge.abbr:"";V.push({subject:ye,className:at,teacherAbbr:ot})}}}});else{const O=Ue(R),_=(Oe=(Re=(Me=M[p.yearKey])==null?void 0:Me[l])==null?void 0:Re[y.num])==null?void 0:Oe[O];(Array.isArray(_)?_:_?[_]:[]).forEach(Y=>{var ce;Y&&V.push({subject:Y.subject,className:Y.className||((ce=Y.classes)==null?void 0:ce.join("+"))||"Klasa",teacherAbbr:Y.teacherAbbr})})}let ie="-";V.length>0&&(ie=V.map(O=>`
                <div style="margin-bottom: 4px; line-height: 1.15;">
                  ...
                  <div style="font-size: ${g}; font-weight: bold; color: #1e293b; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: ${I}; margin-top: 1px;" title="${O.subject}">
                    ${O.subject}
                  </div>
                  ${O.teacherAbbr?`
            ...
            <tr>
              <td style="border: 1px solid #cbd5e1; padding: 6px 4px; text-align: center; font-family: monospace; background-color: #f8fafc; font-weight: bold; font-size: 10px; width: 70px;">
                <div style="font-size: 11px; font-weight: 900; color: #0f172a;">${y.num}</div>
                <div style="font-size: 8px; color: #64748b; margin-top: 1px;">${y.start}-${y.end}</div>
              </td>
              ${se}
            </tr>
          `}),n+=`
          <div class="category-section" style="page-break-inside: avoid; break-inside: avoid; margin-bottom: 24px;">
            <div style="background-color: #f1f5f9; border-left: 4px solid #0f172a; padding: 6px 10px; margin-bottom: 8px; font-weight: bold; font-size: 11px; color: #1e293b; display: flex; align-items: center; gap: 6px; -webkit-print-color-adjust: exact; print-color-adjust: exact;">
              ...
              <span style="letter-spacing: 0.03em;">${r.name.toUpperCase()} (Sal: ${a})</span>
            </div>
            ...
            <table style="width: 100%; border-collapse: collapse; font-family: system-ui, -apple-system, sans-serif; margin-bottom: 12px; table-layout: fixed;">
              <thead>
                ...
                <tr style="background-color: #f8fafc; -webkit-print-color-adjust: exact; print-color-adjust: exact;">
                  ...
                  ${r.cols.map(y=>{const D=y.room.sub||"sala ogólna";return`
                      <th style="border: 1px solid #cbd5e1; padding: ${x}; text-align: center; font-size: 11px; font-weight: 950; min-width: ${i}; color: #020617;">
                        <span style="font-family: monospace; font-size: ${N}; display: block;">🚪 ${y.room.num}</span>
                        ${v?`<span style="font-size: ${$}; color: #475569; font-weight: 500; display: block; margin-top: 1px; text-transform: lowercase;">(${D})</span>`:""}
                      </th>
                    `}).join("")}
                </tr>
              </thead>
        ...
        <div class="day-sheet" style="page-break-after: always; margin-bottom: 30px;">
          <div style="background-color: #0f172a; color: #ffffff; padding: 10px 14px; border-radius: 6px; display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; -webkit-print-color-adjust: exact; print-color-adjust: exact;">
            <span style="font-size: 12px; font-weight: 950; letter-spacing: 0.05em;">
              📅 ${L[l].toUpperCase()} — PŁACHTA OBŁOŻENIA GABINETÓW
            </span>
            <span style="font-size: 9px; font-weight: bold; font-family: monospace; opacity: 0.85;">
              Wydruk podzielony na kategorie (Razem sal: ${De.length})
            </span>
          </div>
      ...
      <body>
        ...
        <div class="header">
          <div class="header-title">
            ...
            <p>${p.school.name} — Rok szkolny ${p.yearLabel}</p>
          </div>
          <div class="meta-info">
            SYSTEM GENERACYJNY SalePlan Pro<br>
            WERSJA: ${z==="etap1"?"PLAN BAZOWY KLAS (ETAP 1)":"PLAN PRZYDZIAŁU SAL (ETAP 2)"}<br>
    ...
    `},ge=()=>{try{const t=Xe(),d=window.open("","_blank");d?(d.document.write(t),d.document.close()):H(!0)}catch(t){console.error(t),H(!0)}},et=()=>{const t=p.dyzury.miejsca,d=p.dyzury.przerwy,s=Math.min(1,Math.max(.45,8/Math.max(t.length,1)));let c="";return[0,1,2,3,4].forEach(l=>{let n="";d.forEach(r=>{let a="";t.forEach(o=>{const b=`${o.id}|${l}|${r.num}`,i=p.dyzury.harmonogram[b],x=i!=null&&i.teacherAbbr?p.teachers.find(u=>u.abbr===i.teacherAbbr):null;let f="-";i!=null&&i.teacherAbbr&&(f=`
              ...
              <div style="font-size: 8.5px; color: #64748b; font-weight: bold; margin-top: 3px; max-width: 100px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; margin-left: auto; margin-right: auto;" title="${x?`${x.first} ${x.last}`:""}">
                ${x?`${x.first.slice(0,1)}. ${x.last}`:"Dyżur"}
              </div>
            `),a+=`
          ...
          <tr>
            <td style="border: 1px solid #cbd5e1; padding: 10px 8px; text-align: left; background-color: #f8fafc; font-weight: bold; font-size: 10.5px; width: 140px;">
              <div style="font-size: 11px; font-weight: 900; color: #0f172a;">${r.name||`Przerwa ${r.num}`}</div>
              <div style="font-size: 8.5px; color: #64748b; font-weight: bold; margin-top: 2px; font-family: monospace;">⏱️ ${r.start} - ${r.end}</div>
            </td>
            ${a}
          </tr>
        `}),c+=`
        <div class="day-section" style="page-break-inside: avoid; break-inside: avoid; margin-bottom: 32px;">
          <div style="background-color: #0f172a; color: #fff; padding: 8px 14px; margin-bottom: 12px; font-weight: 900; font-size: 11.5px; border-radius: 8px; display: flex; align-items: center; justify-content: space-between; -webkit-print-color-adjust: exact; print-color-adjust: exact;">
            <span style="letter-spacing: 0.05em; text-transform: uppercase;">📅 ${L[l]} — HARMONOGRAM DYŻURÓW</span>
                ...
                <tr style="background-color: #f1f5f9; border-bottom: 2px solid #cbd5e1;">
                  ...
                  ${t.map(r=>`
                    <th style="border: 1px solid #cbd5e1; padding: 8px 6px; text-align: center; font-size: 10.5px; font-weight: 900; color: #1e293b; background-color: #f8fafc;">
                      <div style="font-weight: 900; text-transform: uppercase; color: #0f172a; font-size: 10.5px;">📍 ${r.name}</div>
                      ${r.floor?`<div style="font-size: 8px; color: #64748b; font-weight: bold; text-transform: uppercase; margin-top: 2px;">${r.floor}</div>`:""}
                    </th>
                  `).join("")}
                </tr>
              </thead>
    ...
    `},tt=()=>{try{const t=et(),d=window.open("","_blank");d?(d.document.write(t),d.document.close()):H(!0)}catch(t){console.error(t),H(!0)}},re=k.useMemo(()=>S==="all"?m.classes:m.classes.filter(t=>t.id===S),[m.classes,S]),ne=k.useMemo(()=>P==="all"?m.teachers:m.teachers.filter(t=>t.id===P),[m.teachers,P]),De=k.useMemo(()=>C==="all"?m.rooms:m.rooms.filter(t=>t.id===C),[m.rooms,C]),le=k.useMemo(()=>{const t=rt(p.floors),d=C==="all"?t:t.filter(a=>{const o=(a.room.num||"").toLowerCase().trim(),b=m.rooms.find(i=>i.name.toLowerCase().trim()===o);return b&&b.id===C}),s=[],c=[],l=[],n=new Map(m.rooms.map(a=>[a.name.toLowerCase().trim(),a]));d.forEach(a=>{const o=(a.room.num||"").toLowerCase().trim(),b=n.get(o),i=p.buildings[a.floor.buildingIdx],x=(b==null?void 0:b.type)==="indywidualne",f=(b==null?void 0:b.type)==="sport"||(i==null?void 0:i.multi)===!0;x?c.push(a):f?l.push(a):s.push(a)});const r=(a,o)=>{const b=a.room.num||"",i=o.room.num||"";return b.localeCompare(i,void 0,{numeric:!0,sensitivity:"base"})};return s.sort(r),c.sort(r),l.sort(r),{main:s,individual:c,sport:l}},[p.floors,p.buildings,m.rooms,C]),Ee=k.useMemo(()=>[{id:"main",name:"Budynek Główny",icon:"🏢",cols:le.main},{id:"individual",name:"Nauczanie Indywidualne",icon:"🗣️",cols:le.individual},{id:"sport",name:"Sale Sportowe",icon:"🏆",cols:le.sport}].filter(t=>t.cols.length>0),[le]);if(xe)return e.jsxs("div",{id:"weekly-print-overlay",className:"fixed inset-0 bg-slate-100/90 backdrop-blur-md z-[9999] overflow-y-auto p-4 md:p-8 font-sans text-slate-800",children:[e.jsx("style",{dangerouslySetInnerHTML:{__html:`
          @media print {

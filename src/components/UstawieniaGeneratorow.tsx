...
import { isClassGrade1 } from '../utils/adaptationDuty';
    ...
    onChangeAppState({
      ...
      dyzury: {
        ...
        settings: {
          ...
          firstGradeAdaptationDurationMonths: 2,
          firstGradeEscortDuration: 15,
          firstGradeCustomClassIds: [],
          countAdaptationInFTE: true
        }
      }
    });
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-5">
            ...
            <div className="space-y-4">
              ...
              <div className="border-t border-slate-100 pt-4 space-y-3">
                <div className="flex items-start gap-3 p-3 bg-amber-50/50 rounded-xl border border-amber-200/70 hover:bg-amber-50 transition">
                  <input 
                    type="checkbox" 
                    id="set_firstGradeAdaptationDuty"
                    className="mt-1 rounded border-amber-300 text-amber-600 focus:ring-amber-500 h-4 w-4 cursor-pointer"
                    checked={appState.dyzury.settings.firstGradeAdaptationDuty !== false}
                    onChange={(e) => updateDutySettings({ firstGradeAdaptationDuty: e.target.checked })}
                  />
                  <label htmlFor="set_firstGradeAdaptationDuty" className="cursor-pointer select-none space-y-0.5 flex-1">
                    <span className="text-xs font-bold text-amber-950 block flex items-center gap-1.5">
                      🎒 Okres adaptacyjny klas 1 (dyżury w sali & odprowadzanie)
                    </span>
                    <span className="text-[10px] text-amber-800/90 block leading-tight">
                      Na początku roku (1–2 miesiące) uczniowie klas 1 spędzają przerwy w swojej sali pod opieką nauczyciela prowadzącego lekcję, a po ostatnich zajęciach nauczyciel odprowadza ich do szatni, świetlicy lub rodziców.
                    </span>
                  </label>
                </div>

                {appState.dyzury.settings.firstGradeAdaptationDuty !== false && (
                  <div className="ml-2 p-3.5 bg-white border border-amber-200/70 rounded-xl space-y-3 text-xs shadow-xs animate-in slide-in-from-top-1">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">
                          📅 Czas trwania okresu adaptacyjnego:
                        </label>
                        <select
                          className="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-200 text-slate-800 font-bold rounded-lg text-xs outline-none cursor-pointer"
                          value={appState.dyzury.settings.firstGradeAdaptationDurationMonths || 2}
                          onChange={(e) => updateDutySettings({ firstGradeAdaptationDurationMonths: parseInt(e.target.value) || 2 })}
                        >
                          <option value={1}>1 miesiąc (Wrzesień)</option>
                          <option value={2}>2 miesiące (Wrzesień i Październik)</option>
                        </select>
                      </div>

                      <div>
                        <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">
                          ⏱️ Czas dyżuru odprowadzającego:
                        </label>
                        <div className="flex items-center gap-2">
                          <input 
                            ...
                            max={30}
                            step={5}
                            className="w-20 px-2.5 py-1.5 bg-slate-50 border border-slate-200 text-slate-800 font-bold rounded-lg text-xs font-mono text-center outline-none"
                            value={appState.dyzury.settings.firstGradeEscortDuration || 15}
                            onChange={(e) => updateDutySettings({ firstGradeEscortDuration: Math.max(5, parseInt(e.target.value) || 15) })}
                          />
                          <span className="text-[11px] font-semibold text-slate-500">minut po lekcji</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-2 pt-1 border-t border-slate-100">
                      <input 
                        type="checkbox"
                        id="set_countAdaptationInFTE"
                        className="mt-0.5 rounded border-slate-300 text-amber-600 focus:ring-amber-500 h-3.5 w-3.5 cursor-pointer"
                        checked={appState.dyzury.settings.countAdaptationInFTE !== false}
                        onChange={(e) => updateDutySettings({ countAdaptationInFTE: e.target.checked })}
                      />
                      <label htmlFor="set_countAdaptationInFTE" className="cursor-pointer select-none">
                        <span className="text-[11px] font-bold text-slate-800 block">Wliczaj czas opieki w salach i odprowadzania do limitu dyżurów</span>
                        <span className="text-[9.5px] text-slate-500 block leading-tight">
                          Zabezpiecza nauczycieli klas pierwszych przed przeciążeniem — generator automatycznie zmniejsza im liczbę dyżurów korytarzowych.
                        </span>
                      </label>
                    </div>
                    ...
                    <div className="pt-1">
                      <span className="text-[10px] font-bold text-slate-500 uppercase block mb-1.5">
                        🏫 Klasy objęte okresem adaptacyjnym:
                      </span>
                      <div className="flex flex-wrap gap-1.5 max-h-28 overflow-y-auto p-2 bg-slate-50 border border-slate-200/60 rounded-lg">
                        {(appState.planLekcji.classes.length > 0 ? appState.planLekcji.classes : appState.classes).map(cls => {
                          const autoGrade1 = isClassGrade1(cls);
                          const customIds = appState.dyzury.settings.firstGradeCustomClassIds || [];
                          const isCustomMode = customIds.length > 0;
                          const isSelected = isCustomMode ? customIds.includes(cls.id) : autoGrade1;
                          ...
                          return (
                            <button
                              ...
                              type="button"
                              onClick={() => {
                                const baseIds = isCustomMode 
                                  ? customIds 
                                  : (appState.planLekcji.classes.length > 0 ? appState.planLekcji.classes : appState.classes).filter(c => isClassGrade1(c)).map(c => c.id);
                                const nextIds = baseIds.includes(cls.id)
                                  ? baseIds.filter(id => id !== cls.id)
                                  : [...baseIds, cls.id];
                                updateDutySettings({ firstGradeCustomClassIds: nextIds });
                              }}
                              className={`px-2 py-1 rounded-md text-[10px] font-bold transition flex items-center gap-1 border cursor-pointer ${
                                isSelected
                            ...
                            >
                              ...
                              <span>{cls.name}</span>
                              {autoGrade1 && <span className="text-[8px] bg-amber-200/70 text-amber-800 px-1 rounded font-normal">Klasa 1</span>}
                            </button>
                          );
                        })}
                      </div>
                      <p className="text-[9px] text-slate-400 mt-1">
                        *Domyślnie system automatycznie kwalifikuje wszystkie klasy pierwsze. Kliknij na klasę, aby ją dodać lub wykluczyć.
                      </p>
                    </div>
                  </div>
                )}
